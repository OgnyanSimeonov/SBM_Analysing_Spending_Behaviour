...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun 20 12:30:06 2024
...
class sbmtm_weights():
    ...
    def make_graph(self,list_texts, documents = None, weight = None, counts=True, n_min = None):
        '''
        ...
        # Assuming len(weight) is the length of the weight array
        weight_index = 0  # Initialize an index for the weight array
        ...
        for i_d in range(D):
            ...
            for word, count in c.items():
                ...
                # Check if there are more weights to assign
                if weight_index < len(weight):
                    if counts:
                        e = g.add_edge(d, w)
                        ecount[e] = weight[weight_index]
                    else:
    ...
    def fit(self,overlap = False, n_init = 1, verbose=False, epsilon=1e-3):
        ...
        if g is None:
            ...
            for i_n_init in range(n_init):
                ...
                #     print(delta)
                #try with an epsilon of 10-3, 10-4
                # add chucnk from AI for health
    ...
    def clusters(self, l=0, n=10):
        ...
        Most common refers to the largest contribution in the group membership vector.
        For the non-overlapping case, each document belongs to one and only one group with prob 1.
    
        Parameters:
        - l: int, the level of the hierarchy to get the groups from (default: 0)
        - n: int, number of top documents to retrieve from each cluster (default: 10)
        
        Returns:
        - dict_group_docs: dict, where keys are group indices and values are lists of tuples (document, probability)
        '''
        # Retrieve the group information for the specified level
        dict_groups = self.get_groups(l=l)
        if not dict_groups:
            raise ValueError("No group information available for the specified level")
        ...
        # Ensure p_td_d has correct dimensions
        if Bd != p_td_d.shape[0] or len(docs) != p_td_d.shape[1]:
            raise ValueError("Mismatch between group dimensions and document count")
        ...
        for td in range(Bd):
            ...
            for i in ind_d_[:n]:
                if p_d_[i] > 0:
                    list_docs_td.append((docs[i], p_d_[i]))
                else:
                    break
